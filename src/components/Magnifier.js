...
const Magnifier = React.createClass({
  ...
  render() {
    ...
    const imageSize = props.zoomImage ? props.zoomImage : getImageSize(props.src);
    ...
    const isVisible = props.offsetY < props.smallImage.height &&
                    props.offsetX < props.smallImage.width &&
                    props.offsetY > 0 &&
                    props.offsetX > 0;
    console.log(`${props.offsetY} < ${props.smallImage.height} && ${props.offsetX} < ${props.smallImage.width} && ${props.offsetY} > 0 && ${props.offsetX} > 0 : ${isVisible}`);
    return (
      <div style={{
        position: 'absolute',
